  ...
  methods: {
    get (dataGet) {
      ...
      Object.values(dataGet).map(function (queue) {
        sumAbandoned = sumAbandoned + queue.abandoned
        sumCalls = sumCalls + queue.calls
      })
      this.d_i_sumAbandoned = sumAbandoned
      this.d_i_sumCalls = sumCalls
      if (sumCalls !== 0) {
        this.d_i_widgetData = (sumAbandoned / sumCalls) * 100
      } else {
        this.d_i_widgetData = 0
      }
    ...
    on (onData, oldData) {
      const queue = oldData
      if (queue !== undefined) {
        const diffAbandoned = parseInt(onData.abandoned) - parseInt(queue.abandoned)
        const diffCalls = parseInt(onData.calls) - parseInt(queue.calls)

        this.d_i_sumAbandoned = this.d_i_sumAbandoned + diffAbandoned
        this.d_i_sumCalls = this.d_i_sumCalls + diffCalls
        this.d_i_widgetData = (this.d_i_sumAbandoned / this.d_i_sumCalls) * 100
      }
      return this.d_i_widgetData
    }
  }
