...
export default {
  ...
  methods: {
    get (dataGet) {
      ...
      this.d_i_widgetData = sumAbandoned
    ...
    on (onData, oldData) {
      const queue = oldData
      if (queue !== undefined) {
        const difAbandoned = parseInt(onData.abandoned) - parseInt(queue.abandoned)
        this.d_i_sumAbandoned = this.d_i_sumAbandoned + difAbandoned
        this.d_i_widgetData = this.d_i_sumAbandoned
      }
      return this.d_i_widgetData
    }
  }
