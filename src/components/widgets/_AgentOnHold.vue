...
export default {
  ...
  methods: {
    ...
    on (onData, oldData) {
      ...
      let chkUpdate = true
      let valDeff = 0
      if (extension.q_login !== onData.q_login || extension.q_status !== onData.q_status) {
        if ((onData.q_login === true && onData.q_status === 8) && (extension.q_status !== 8 || extension.q_login !== true)) {
          chkUpdate = true
          valDeff = 1
        } else if ((onData.q_login !== true || onData.q_status !== 8) && (extension.q_status === 8 && extension.q_login === true)) {
          chkUpdate = false
          valDeff = -1
        }
