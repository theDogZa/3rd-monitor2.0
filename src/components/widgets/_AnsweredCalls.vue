...
export default {
  ...
  methods: {
    get (dataGet) {
      ...
      this.d_i_widgetData = sumCompleted
    ...
    on (onData, oldData) {
      ...
      if (Object.keys(queue).length > 0) {
        const difCompleted = parseInt(onData.completed) - parseInt(queue.completed)
        this.d_i_sumCompleted = this.d_i_sumCompleted + difCompleted
        this.d_i_widgetData = this.d_i_sumCompleted
      }
      return this.d_i_widgetData
    }
  }
